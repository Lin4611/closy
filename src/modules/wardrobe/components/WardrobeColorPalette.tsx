        ...
        {visibleColors.map((color) => {
          ...
          return (
            <button
              ...
              className={cn(
                'rounded-[12px] p-1.5 transition-all',
                selected ? 'bg-[#C9B089]' : 'bg-transparent',
            ...
            >
              <div className="overflow-hidden rounded-[10px] border border-neutral-300 bg-white">
                <div className="grid h-15 w-15 grid-cols-2">
                  ...
                  <span className="border border-white/70" style={{ backgroundColor: color.hex }} />
                  <span
                    className="border border-white/70 opacity-90"
                    style={{ backgroundColor: color.hex }}
                  />
                  <span
                    className="border border-white/70 opacity-80"
                    style={{ backgroundColor: color.hex }}
                  />
                </div>
              </div>
              <span
                className={cn(
                  'mt-1 inline-flex rounded-full px-2 py-0.5 font-label-xxs-sb',
                  color.textClassName ?? 'text-white'
                )}
                style={{ backgroundColor: color.hex }}
              >
                {color.label}
